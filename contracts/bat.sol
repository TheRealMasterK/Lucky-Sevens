...
contract BatmanToken is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("BATMAN", unicode"BATMAN") {
        ...
        _mint(msg.sender, 1000000000000000000000000); // 1,000,000 BAT tokens minted
        rewardRate = 1000; // Reward rate per block
    ...
    /// Check this line for continuation
    ...
    mapping(address => uint256) public staked; // Tokens staked by each user
    mapping(address => uint256) public lastBlockClaimed; // Last block a user claimed rewards
    ...
    function unstake(uint256 amount) external nonReentrant {
        require(amount > 0, "Amount must be greater than 0");
        require(staked[_msgSender()] >= amount, "Not enough tokens staked");
        claimRewards();
    ...
    function claimRewards() public nonReentrant {
        ...
        if (pendingRewards > 0) {
            _mint(_msgSender(), pendingRewards);
            lastBlockClaimed[_msgSender()] = block.number;
            emit RewardsClaimed(_msgSender(), pendingRewards);
        }
    }

    function getPendingRewards(address _user) public view returns (uint256) {
        uint256 stakedTokens = staked[_user];
        uint256 lastClaimed = lastBlockClaimed[_user];
        uint256 pendingBlocks = block.number - lastClaimed;
        return stakedTokens * pendingBlocks * rewardRate;
    }
}
